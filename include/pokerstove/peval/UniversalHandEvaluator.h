...
namespace pokerstove
{
    ...
    class UniversalHandEvaluator : public PokerHandEvaluator
    {
       public:
         ...
         */
        UniversalHandEvaluator(int heromin, int heromax, int boardmin, int boardmax, int herouse, evalFunction evalA, evalFunction evalB)
        ...
        virtual PokerHandEvaluation evaluateHand(const CardSet& hand, const CardSet& board) const
        {
            ...
            if (hand.size() < _heromin || hand.size() > _heromax)
                throw std::invalid_argument(std::string("UnivHandEval: " + boost::lexical_cast<std::string>(uint(hand.size())) +
            ...
            if ((bz < _boardmin && bz > 0) || bz > _boardmax)
                throw std::invalid_argument(std::string("UnivHandEval: " + boost::lexical_cast<std::string>((uint)board.size()) +

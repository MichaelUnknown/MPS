...
namespace pokerstove
{
    ...
    struct EquityResult
    {
        ...
        explicit EquityResult() : winShares(0.0), tieShares(0.0), equity(0.0), equity2(0.0) {}
        ...
        {
            std::string ret = boost::lexical_cast<std::string>(winShares) + " " + boost::lexical_cast<std::string>(tieShares) + " " +
                              boost::lexical_cast<std::string>(equity) + " " + boost::lexical_cast<std::string>(equity2);
            return ret;
        }
    };
    ...
    class PokerHandEvaluator
    {
       public:
         ...
         */
        virtual PokerHandEvaluation evaluateHand(const CardSet& hand, const CardSet& board = CardSet(0)) const = 0;

        virtual PokerHandEvaluation evaluate(const CardSet& hand, const CardSet& board = CardSet(0)) { return evaluateHand(hand, board); }

        /**
         * This version is for when you know you are only interested in
         * non-split pot games.
         */
        virtual PokerEvaluation eval(const CardSet& hand, const CardSet& board = CardSet(0)) { return evaluateHand(hand, board).high(); }
         ...
         */
        virtual void setNumDraws(size_t sz) { throw std::runtime_error("not implemented"); }
         ...
         */
        void evaluateShowdown(const std::vector<CardSet>& hands, const pokerstove::CardSet& board, std::vector<PokerHandEvaluation>& evals,
                              std::vector<EquityResult>& result, double weight = 1.0) const;

...
#define LOWBALL_ROTATE_RANKS(ranks) ((((ranks) & ~(1 << Rank::AceVal())) << 1) | (((ranks) >> Rank::AceVal()) & 0x01))
...
CardSet::CardSet(const string& c) : _cardmask(0)
{
    fromString(c);
}
...
{
    return CardSet(static_cast<uint64_t>(suitMask(Suit::Clubs())) << Rank::NUM_RANK * c |
...
CardSet CardSet::canonize() const
{
    ...
    return CardSet(static_cast<uint64_t>(smasks[3]) | static_cast<uint64_t>(smasks[2]) << Rank::NUM_RANK |
                   static_cast<uint64_t>(smasks[1]) << Rank::NUM_RANK * 2 | static_cast<uint64_t>(smasks[0]) << Rank::NUM_RANK * 3);
}
...
PokerEvaluation CardSet::evaluateBadugi() const
{
    ...
    std::array<int, 4> suits = {
        {LOWBALL_ROTATE_RANKS(C()), LOWBALL_ROTATE_RANKS(D()), LOWBALL_ROTATE_RANKS(H()), LOWBALL_ROTATE_RANKS(S())}};
